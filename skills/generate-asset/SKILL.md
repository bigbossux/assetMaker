...
6. **Prefer cheap validation first.** If testing whether a prompt fix works, use a shorter duration / lower resolution / single test frame before committing to a full-price full-resolution run — and still show the price confirmation for that cheap test, not just the final run. But don't treat the full-price run as a pure quality upsize of the validated one: these models have no seed/determinism parameter, so resubmitting identical `reference_images`/`reference_audios`/`prompt` at a higher resolution is a fresh generation, not a re-render — it can shift lip-sync timing, drop or change background details (e.g. animated UI elements), and reintroduce limb/artifact hallucinations that the validated version didn't have. Re-run the same lipsync/composition/artifact verification on the final render before shipping it; don't assume it carries over.
...
**In Cowork, the plugin-bundled `mcp__atlascloud__*` tools will never work** (other than `atlas_get_model_info`/`atlas_search_docs`, which are read-only and auth-independent, and work regardless) — Atlas Cloud's MCP server is a local-process/stdio server with no remote/hosted equivalent, and that specific connector has no working way to configure a real key into it (ruled out by direct testing: read-only after install, no reinstall-time prompt). **But check first whether the user has set up `setup`'s Path B2** (the self-hosted `cloudflare-mcp-proxy` Worker, added as a separate custom connector) — if so, real working tools should be available under whatever name that connector's tools actually show up as; use those normally rather than falling back to curl. If no such connector is configured, go straight to direct `curl` calls (see `setup`'s Path B1 for the full pattern — reading the key from a `.env` file in the mounted project folder, since there's no persistent shell to export it in ahead of time):
```bash
...
- **Character/limb hallucination**: reference-to-video models can invent extra limbs (e.g. hands/arms on a character with none) especially during "gesture" instructions. If a character's design has no arms, say so explicitly and forbid it in the prompt ("NEITHER mascot has arms — do not draw any arm/hand/finger shape, ever"), and describe gestures as body lean/tilt instead of limb movement. Apply this to *every* character in the scene, not just the one that failed last time — the model will happily move the artifact to a different character. This can also reappear on a *later* render that previously came out clean (e.g. a validated 720p pass, then a 1080p resubmission with identical inputs) — check every new render, not just the first one.
- **No determinism across resubmissions**: there's no seed parameter, so resubmitting the exact same `reference_images`/`reference_audios`/`prompt` (even just to bump resolution) is a fresh independent generation, not a re-render of the same result at higher fidelity. Observed on one project: a 720p→1080p resubmission with identical inputs shifted the lip-sync timing window by ~0.15s *and* dropped an animated background detail (UI cards highlighting in sync with dialogue, present at 720p, absent at 1080p — the model regenerates the whole scene, not just upsamples it). Budget for a full re-verification pass (lip-sync alignment, key background/interaction details, limb check) on the final-resolution render, not just the validation one.
...
- **Multi-speaker audio**: if a scene has two speakers, pass each speaker's line as a *separate* `reference_audios` entry (don't merge into one track) — models given a single merged track will guess where the speaker change happens and often get it wrong. If you only have one pre-existing merged track to work with (rather than generating clean separate lines from scratch — see "Before generating video specifically" above, which is the better path when starting fresh), split it precisely using word-level transcription timestamps rather than guessing from silence gaps alone: silence-based splitting catches pauses but not necessarily *which* pause is the actual speaker change. Local transcription options, in order of preference:
  1. `pip install openai-whisper` — pure Python, works in both Claude Code CLI and Cowork's Linux sandbox.
  2. `brew install whisper-cpp` — faster, but Homebrew is macOS-native and likely unavailable in Cowork's Linux VM sandbox (unconfirmed either way by Anthropic's docs — assume it may fail there). Fine to use in a regular Claude Code CLI session on macOS.
- **Image-edit models anchor hard to the reference's existing composition** (seen on `google/nano-banana-pro/edit`): asking it to rearrange a reference image's layout (e.g. "turn this tall vertical stack into a horizontal lockup") is often just ignored — it extends/pads the background but leaves the original composition where it was. A vague instruction like "arrange horizontally" isn't enough. Give hard, quantified constraints instead (e.g. "the combined lockup must be no taller than 28% of canvas height, centered vertically, spanning roughly the middle 55% of canvas width") and still expect to need one retry with an even more explicit prompt before it complies.
- **"Transparent background" often isn't real alpha**: asking an image-edit model for a transparent background can produce a visible black/white checkerboard pattern baked into the actual RGB pixels — not a real alpha channel — even though the prompt was followed as asked. Always verify with `sips -g hasAlpha file.png` (or equivalent) before trusting a "transparent" result. Better workflow: don't ask the generation model for transparency at all — generate on a solid plain background color, then run a dedicated background-removal model (`youchuan/v8.1/remove-background`, $0.086/request) as a separate step. This matters especially when the foreground art contains black or white elements of its own (outlines, highlights) — a baked-in checkerboard's black/white squares get confused with matching-color linework by the removal pass and leave residue around edges, whereas a solid background color segments cleanly.
- **PROHIBITED_CONTENT false positives**: `google/nano-banana-pro/edit` can reject an innocuous prompt (e.g. simplifying a cute mascot logo into a flat icon, no sensitive content involved) with `no parts found for the request: PROHIBITED_CONTENT`. Don't try to diagnose which phrase triggered it — retry once with a shorter, simpler prompt; that alone has resolved it. Whether the failed attempt consumes credits wasn't confirmed — check balance before and after if it matters for the budget in play.
